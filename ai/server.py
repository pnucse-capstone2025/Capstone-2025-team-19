...
async def handle_pdf(
    ...
    sessionId: str = Form(...)      # session_id -> sessionId
):
    ...
    try:
        app_backend.task_manager.create_context_task(
            user_id=user.id, docs_path=saved_path, session_id=sessionId
        )
        print(f"[PDF] ContextTask 등록됨: user={userId}, session={sessionId}, path={saved_path}")
    except Exception as e:
...
async def handle_text(body: TextRequest):
    ...
    if body.chatId:
        ...
        if not getattr(user, "chats", None):
            raise HTTPException(
                status_code=400,
                detail="해당 유저의 Chat이 없습니다. /pdf 업로드로 컨텍스트부터 생성하세요."
            )
        chat_id = user.chats[-1].id

    # 메시지를 큐잉할 dict 형태로 변환
    task = {
        "userId": body.userId,
    ...
    # ✅ 직접 실행 대신 TaskManager에만 등록
    app_backend.task_manager.create_chat_task(user_id=body.userId, msg=task)
    ...
    return JSONResponse(
        {
            "success": True,
            "status": "queued",
            "jobId": str(uuid.uuid4()),   # 임시 JobID 발급 (추적용)
            "sessionId": body.sessionId,
            "seq": body.seq,
        },
        status_code=202,
...
if __name__ == "__main__":
    uvicorn.run("server:app", host="0.0.0.0", port=int(PORT_NUM), reload=True)
